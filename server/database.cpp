...
bool Database::createPosition(string symName, string accountId, float amount){
    ...
    while(marker != true){
        try{
            ...
            pqxx::result R1( N.exec(check_postion_owner_valid_sql));
            ...
            pqxx::result R2( N.exec(check_postion_exist_sql));      
...
int Database::createOrder(string symName, string accountId, float amount, float limit){
    ...
    while(marker != true){
        try{
            ...
            pqxx::result R( W.exec(get_trans_id_sql));
            W.commit();
            pqxx::result::const_iterator c = R.begin();
    ...
    while(marker != true){
        ...
        try{
            if(amount > 0){//for buy, amount > 0
                ...
                //pqxx::nontransaction N1(*C);
                pqxx::result R1(W.exec(find_match_sell_sql));
                W.commit();
                marker = true;
                ...
                //pqxx::nontransaction N1(*C);
                pqxx::result R1(W.exec(find_match_buy_sql));
                W.commit();
                marker = true;
    ...
    while(marker != true){
        try{
            if(amount <= 0){// sell order
                time_t currTime = time(NULL);
                string creat_execution_order_sql = "INSERT INTO ORDER_EXECUTION (OWNER_ID, BUYER_ID, SELLER_ID, BUY_ID, SELL_ID, SYM, PRICE, AMOUNT, TIME) VALUES (" +\
                                                    sellAccountId + ", " + buyAccountId + ", " + sellAccountId + \
                ...
                time_t currTime = time(NULL);
                string creat_execution_order_sql = "INSERT INTO ORDER_EXECUTION (OWNER_ID, BUYER_ID, SELLER_ID, BUY_ID, SELL_ID, SYM, PRICE, AMOUNT, TIME) VALUES (" +\
                                                    buyAccountId + ", " + buyAccountId + ", " + sellAccountId + \
                                                    ", " + buyTransId + ", " + sellTransId + \
                                                    ", '" + symName + "', " + to_string(price) +		\
                                                    ", " + to_string(amount) + ", " + to_string(currTime) + ");";
                //pqxx::work W1(*C);
...
bool Database::cancel(string accountId, string transId, vector<CancelOrder> &cancelOpenSet, vector<ExecutedOrder> &cancelExecutedSet){
    ...
    while(marker != true){
        try{
            ...
            //pqxx::nontransaction N1(*C);
            pqxx::result R1( W.exec(open_order_find_sql));  
            //N1.commit();
            ...
            string execute_order_find_sql = "SELECT PRICE, AMOUNT, TIME from ORDER_EXECUTION WHERE (OWNER_ID = " + accountId + ") " + "AND (BUY_ID = " + transId + " OR SELL_ID = " + transId + ");";
            //pqxx::nontransaction N2(*C);
            pqxx::result R2( W.exec(execute_order_find_sql));  
            W.commit();
            for(pqxx::result::const_iterator c2 = R2.begin(); c2 != R2.end(); ++c2){
                float cancelExcutedPrice= c2[0].as<float>();
                float cancelExecutedShare = c2[1].as<float>();
                time_t cancelExecutedTime = c2[2].as<time_t>();
...
bool Database::query(string accountId, string transId, vector<OpenOrder> &queryOpenSet, vector<CancelOrder> &queryCancelSet, vector<ExecutedOrder> &queryExecutedSet){
    ...
    while(marker != true){
        try{
            string query_check_account_exist = "SELECT ACCOUNT_ID from ACCOUNT WHERE ACCOUNT_ID = " + accountId + ";" ;
            //pqxx::nontransaction N4(*C);
            pqxx::result R4( W.exec(query_check_account_exist));
            //N4.commit();
            ...
            //pqxx::nontransaction N1(*C);
            pqxx::result R1( W.exec(query_open_order_find_sql));  
            //N1.commit();
            for(pqxx::result::const_iterator c = R1.begin(); c != R1.end(); ++c){
            ...
            string query_execute_order_find_sql = "SELECT PRICE, AMOUNT, TIME from ORDER_EXECUTION WHERE (OWNER_ID = " + accountId + ") " + "AND (BUY_ID = " + transId + " OR SELL_ID = " + transId + ");";
            //pqxx::nontransaction N3(*C);
            pqxx::result R3( W.exec(query_execute_order_find_sql));  
            W.commit();
            for(pqxx::result::const_iterator c = R3.begin(); c != R3.end(); ++c){
                float queryExcutePrice= c[0].as<float>();
                float queryExcuteShare = c[1].as<float>();
                time_t queryExcuteTime = c[2].as<time_t>();
...
bool Database::checkAccountValid(string accountId){
    ...
    while(marker != true){
        try{
            ...
            //pqxx::nontransaction N(*C);    
            pqxx::result R( W.exec(check_account_sql));
            W.commit();
            marker = true;
            pqxx::result::const_iterator c = R.begin(); 
            if( c == R.end()){     
...
bool Database::checkBalance(string accountId, float amount, float limits){
    ...
    while(marker != true){
        try{
            ...
            //pqxx::nontransaction N(*C);    
            pqxx::result R( W.exec(check_balance_sql));
            //N.commit();
            pqxx::result::const_iterator c = R.begin(); 
            if( c == R.end()){     
                W.commit();
...
bool Database::checkAmount(string accountId, float amount, string symName){
    ...
    while(marker != true){
            ...
            string check_amount_sql = "SELECT AMOUNT FROM POSITIONS WHERE ACCOUNT_ID = " + accountId + \
                                        " AND SYM='" + symName +"';";
            //pqxx::nontransaction N(*C);    
            pqxx::result R( W.exec(check_amount_sql));
            //N.commit();
            pqxx::result::const_iterator c = R.begin(); 
            if( c == R.end()){  
                W.commit(); 
